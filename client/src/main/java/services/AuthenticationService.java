...
public class AuthenticationService {
    ...
    public boolean register(String username, String password) {
        ...
        try {
            restTemplate.postForObject(API_BASE_URL + "register", makeRegisterUserDTO(registerUserDTO), Void.class);
            success = true;
        } catch (RestClientException e) {
            System.out.println("Error. Was not able to register. " + e.getMessage());
        }
        return success;
    }

    private HttpEntity<RegisterUserDTO> makeRegisterUserDTO(RegisterUserDTO registerUserDTO) {
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        //headers.setBearerAuth(authToken);
        return new HttpEntity<>(registerUserDTO, headers);
    }
